...
interface ReportUserItemProps {
  report: ReportDTO;
}
const ReportUserItem = ({ report }: ReportUserItemProps) => {
  const checkNotPendingStatus =
  ...
  return (
    <div className="bg-white p-4 mt-5 space-y-4 shadow-md rounded-xl">
      {report.post && (
        ...
        <div>
          ...
          <div className="border mt-4">
            <div className="p-2">
              ...
              <div className="bg-gray-50 p-3 rounded-md border border-black my-4">{report.comment.content}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between">
        <div className="flex space-x-2 items-center">
          <p>Trạng thái:</p>
          <p className="text-lg font-semibold">{nameStatus()}</p>
        </div>
        {checkNotPendingStatus && (
          <div className="flex space-x-2 items-center">
            <p>Thời gian xét duyệt:</p>
            <p>{new Date(report.createdAt).toLocaleString("vi-VN")}</p>
          </div>
        )}
      </div>
